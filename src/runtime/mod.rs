...
//! in three levels. See crate documentation for a description of each.
//!
//! # Safe API usage
//!
...
//! device.dtoh_sync_copy_into(&a_dev, &mut a_buf).unwrap();
...
//! for i_batch in 0..3 {
//!     let mut a_sub_view: CudaViewMut<f32> = a.try_slice_mut(i_batch * 10..(i_batch + 1) * 10).unwrap();
...
//!
//! The first part of safety is ensuring that [crate::runtime::sys::cudaStream_t],
//! and [crate::runtime::sys::cudaDeviceProp] all
...
//! This ensures that data isn't mutated by more than one stream at a time and also
