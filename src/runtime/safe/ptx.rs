use crate::{
    driver::result,
    nvrtc::{Ptx, PtxKind},
    runtime::{sys, RuntimeError},
};

use super::core::CudaDevice;
use crate::driver::core::CudaModule;
use std::ffi::CString;
...
impl CudaDevice {
    ...
    pub fn load_ptx(
        ...
        let cu_module = cu_module.unwrap();
        ...
        for &fn_name in func_names.iter() {
            ...
            if cu_function.is_err() {
                return Err(RuntimeError(sys::cudaError::cudaErrorInvalidValue));
            }
            functions.insert(fn_name, cu_function.unwrap());
        }
