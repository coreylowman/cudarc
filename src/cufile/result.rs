...
pub unsafe fn cufile_driver_open() -> Result<(), CufileError> {
    sys::cuFileDriverOpen().result()
}

pub unsafe fn cufile_driver_get_properties() -> Result<sys::CUfileDrvProps, CufileError> {
    let mut props = sys::CUfileDrvProps::default();
    sys::cuFileDriverGetProperties(&mut props).result()?;
    Ok(props)
}

pub unsafe fn cufile_driver_close() -> Result<(), CufileError> {
    sys::cuFileDriverClose_v2().result()
}

pub unsafe fn cufile_handle_register(fd: i32) -> Result<sys::CUfileHandle_t, CufileError> {
