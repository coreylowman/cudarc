...
mod tests {
    ...
    fn test_pooling() {
        ...
        let input = Tensor4D::alloc_with(&device, [[[[1.0f64, 2.0, 3.0], [-1.0, 0.0, -1.0], [0.0, 3.0, 5.0]]]]).unwrap();
        ...
        pooling2d.forward(&cudnn_handle, &input, &mut output).unwrap();

        assert_eq!(output.get_data().as_host().unwrap()[0][0], [[2.0, 3.0], [3.0, 5.0]]);

        pooling2d.backward(&cudnn_handle, &input, &output, &output, &mut dx).unwrap();

        assert_eq!(dx.get_data().as_host().unwrap()[0][0], [[0.0, 2.0, 3.0], [0.0, 0.0, 0.0], [0.0, 3.0, 5.0]]);
    }
}