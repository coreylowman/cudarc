...
pub struct GmmConfig {
    ...
    /// alpha for each group (len = group count)
    /// TODO this type should match the compute type of the handle
    pub alpha_array: Vec<f32>,

    /// beta for each group (len = group count)
    /// TODO this type should match the compute type of the handle
    pub beta_array: Vec<f32>,
...
#[cfg(feature = "f16")]
impl Gmm<half::f16> for CudaBlas {
    fn gmm_raw(
        &self,
        config: GmmConfig,
        a_ptrs: *const *const c_void,
        ...
        let cuda_dtype = sys::cudaDataType_t::CUDA_R_16F;
        let group_count = config.group_count();

        // For CUBLAS_COMPUTE_32F, alpha and beta must be f32
        let alpha_f32: Vec<f32> = config.alpha_array.iter().map(|x| *x as f32).collect();
        let beta_f32: Vec<f32> = config.beta_array.iter().map(|x| *x as f32).collect();
    ...
    fn gmm(
        &self,
        config: GmmConfig,
        a_slices: &[&CudaSlice<half::f16>],
        b_slices: &[&CudaSlice<half::f16>],
        c_slices: &[&CudaSlice<half::f16>],
    ) -> Result<(), CublasError> {
...
// TODO mostly copy paste from f16, probably use a macro
#[cfg(feature = "f16")]
impl Gmm<half::bf16> for CudaBlas {
    fn gmm_raw(
        &self,
        config: GmmConfig,
        a_ptrs: *const *const c_void,
        ...
        let cuda_dtype = sys::cudaDataType_t::CUDA_R_16BF;
        let group_count = config.group_count();
    ...
    fn gmm(
        &self,
        config: GmmConfig,
        a_slices: &[&CudaSlice<half::bf16>],
        b_slices: &[&CudaSlice<half::bf16>],
        c_slices: &[&CudaSlice<half::bf16>],
    ) -> Result<(), CublasError> {
        config.validate();
